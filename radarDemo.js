!function() {
   ...
   setTimeout(function() { 
      operation.html(" radarChart.options({'legend': true}); <br> radarChart.colors({'iPhone': 'blue', 'Samsung': 'red', 'Nokia Smartphone': 'yellow'}).update(); ");
      radarChart.options({'legend': true});
      radarChart.colors({'iPhone': 'blue', 'Samsung': 'red', 'Nokia Smartphone': 'yellow'}).update();
   }, 4000);
   
   setTimeout(function() { 
      operation.text(" radarChart.options({circles: {maxValue: 1, levels: 4}}).update(); ");
   ...
   setTimeout(function() { 
      operation.text(" radarChart.invert([7, 'Brand']).update(); ");
      radarChart.invert([7, 'Brand']).update();
   }, 16000);

   setTimeout(function() { 
      operation.text(" radarChart.ranges({'Brand': [0, .4], 'Contract Cost': [-1, 2]}).update(); ");
      radarChart.ranges({'Brand': [0, .4], "Contract Cost": [-1, 2]}).update();
   }, 20000);
   ...
   setTimeout(function() { 
      operation.html(" radarChart.ranges({'Brand': [], 'Contract Cost': []}).invert([7, 'Brand']); <br> radarChart.data(data).update(); ");
      radarChart.ranges({'Brand': [], 'Contract Cost': []}).invert([7, 'Brand']);
   ...
   setTimeout(function() { 
      operation.text(" radarChart.options({circles: {maxValue: 1, levels: 3}, legendSymbol: 'circle', filter: false}).update(); ");
      radarChart.options({circles: {maxValue: 1, levels: 3}, legendSymbol: 'circle', filter: false}).update();
   }, 40000);

   setTimeout(function() { 
      operation.text(" radarChart.height(300).width(300).updateDimensions(); ");
      radarChart.height(300).width(300).update();
   }, 44000);

   setTimeout(function() { 
      operation.text(" radarChart.height(600).width(600).updateDimensions(); ");
      radarChart.height(600).width(600).update();
   }, 48000);

   setTimeout(function() { 
      operation.html(" radarChart.options({circles: {fill: '#CDCDCD', color: '#CDCDCD'}}); <br> radarChart.options({axes: {lineColor: 'white'}}); <br> radarChart.colors({}).data(data).update(); ");
      radarChart.options({circles: {fill: '#CDCDCD', color: '#CDCDCD'}});
      radarChart.options({axes: {lineColor: 'white'}, filter: 'glow'});
      radarChart.maxValue(.5).levels(7);
      radarChart.colors({}).data(data).update();
   }, 52000);

   setTimeout(function() { 
      operation.text(" radarChart.data(data2).update(); ");
      radarChart.data(data2).update();
   }, 56000);

   setTimeout(function() { 
      operation.text(" radarChart.data(data).update(); ");
      radarChart.data(data).update();
   }, 60000);
